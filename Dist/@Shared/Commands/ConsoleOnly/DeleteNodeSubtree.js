...
==========
*/
export function GetNodesInSubtree(rootNodeID, runInfo = { nodesVisited: new Set() }) {
    if (runInfo.nodesVisited.has(rootNodeID))
        return [];
    ...
    for (const childID of CE(rootNode.children || {}).VKeys()) {
        ...
        if (subtreeNodes == null)
            return null; // if any subtree contents are not yet loaded, have the whole call resolve to null
...
export class DeleteNodeSubtree extends Command {
    Validate() {
        AssertValidate({
            properties: {
                nodeID: { type: "string" },
